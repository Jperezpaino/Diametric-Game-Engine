...
Steps in order:
- **4a** Camera offset (WorldPoint → screen with offset, follow player).
- **4b** Zoom (`AffineTransform`, UI key binding).
- **4c** A\* on `TileLayer` voxel graph.
- **4d** Click-to-move (screen→world via inverse camera transform).
