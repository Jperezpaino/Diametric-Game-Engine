...
- **7c** Entity sprite pipeline — `EntityRenderer` loads real PNG atlas; fallback to `PlaceholderSprite` if asset missing. ⏳ Pending.

## Phase 8 — Map format spec & editor architecture
Define the canonical map format and architect the editor as a **standalone tool** (never bundled with the game client) before writing a single line of editor UI.

- **8a** Map format spec — formalise JSON schema with versioning, metadata (name, author, tileset ref), and asset-naming conventions. ⏳ Pending.
- **8b** Editor architecture design — separate Maven module / executable; shared `map.io` kernel; no editor code reachable from the game runtime. ⏳ Pending.
- **8c** Editor implementation — palette panel, paint/erase/fill ops, layer selector, undo/redo, `JFileChooser` save/load. ⏳ Pending.

## Phase 9 — Map editor & persistence (future)
