...
public final class Camera {
    ...
    /**
     * Inverse of {@link #apply(Graphics2D)}: maps a screen pixel back to the
     * world-space screen pixel that {@link IsoProjection#screenToWorld} can
     * convert into a (col, row) ground coordinate. Used for click-to-move
     * (Phase 4d).
     *
     * @param sx horizontal pixel in screen coordinates
     * @param sy vertical pixel in screen coordinates
     * @return equivalent point in world-space screen pixels
     */
    public ScreenPoint screenToWorldPixel(final int sx, final int sy) {
        final double zoom  = getZoom();
        final double sX    = GameConfig.SCREEN_WIDTH  / 2.0;
        final double sY    = GameConfig.SCREEN_HEIGHT / 2.0;
        final double snapX = Math.round(offsetX * zoom) / zoom;
        final double snapY = Math.round(offsetY * zoom) / zoom;
        final double wx = (sx - sX) / zoom + sX - snapX;
        final double wy = (sy - sY) / zoom + sY - snapY;
        return new ScreenPoint((int) Math.round(wx), (int) Math.round(wy));
    }
