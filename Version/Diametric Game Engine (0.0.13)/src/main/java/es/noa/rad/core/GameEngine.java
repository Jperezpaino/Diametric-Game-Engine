...
public final class GameEngine {
    ...
    private void handleDebugPathfinding() {
        ...
        if (down && !prevPathDown) {
            ...
            final int tc = (fc < map.getWidth() / 2) ? map.getWidth() - 1  : 0;
            final int tr = (fr < map.getDepth() / 2) ? map.getDepth() - 1  : 0;
            debugPath = pathfinder.findPath(fc, fr, tc, tr);
            System.out.println("[Pathfinder] (" + fc + "," + fr + ") -> ("
                + tc + "," + tr + ") = " + debugPath.size() + " steps");
        }
        prevPathDown = down;
    }
