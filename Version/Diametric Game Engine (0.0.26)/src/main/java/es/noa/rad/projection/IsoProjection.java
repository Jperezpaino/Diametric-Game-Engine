...
public final class IsoProjection {
    ...
    /**
     * Picks the cell whose top surface contains the world-screen pixel,
     * accounting for tile elevation. Iterates from the topmost candidate Z
     * down to 0 and accepts the first column whose top tile has matching
     * elevation. Returns {@code null} if no tile is hit.
     *
     * @param sx        horizontal world-screen pixel
     * @param sy        vertical world-screen pixel
     * @param layer     tile layer to query
     * @param maxZScan  upper bound for elevation scan (inclusive)
     * @return {@code int[]{col, row}} of the picked top tile, or {@code null}
     */
    public int[] pickCell(final int sx, final int sy,
                          final es.noa.rad.map.TileLayer layer,
                          final int maxZScan) {
        for (int z = maxZScan; z >= 0; z--) {
            final int compensatedSy = sy + z * zStep;
            final float a = (float) sx / halfTileWidth;
            final float b = (float) compensatedSy / halfTileHeight;
            final int col = Math.round((a + b) / 2f);
            final int row = Math.round((b - a) / 2f);
            final es.noa.rad.map.Tile top = layer.getTopTile(col, row);
            if (top != null && top.elevation() == z) {
