...
public final class GameEngine {
    ...
    private void handleMouseClick() {
        ...
        final WorldPoint  wp = projection.screenToWorld(worldPx.getX(), worldPx.getY());
        final int tc = Math.round(wp.getCol());
        final int tr = Math.round(wp.getRow());
    ...
    public void render() {
        ...
        try {
            ...
            renderer.render(g, map, player, camera, loop.getCurrentFps(), loop.getCurrentUps(), overlay);
        } finally {
            g.dispose();
        }
        window.show();
