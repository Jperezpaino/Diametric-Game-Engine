...
`GameRenderer` muestra dos líneas en espacio de pantalla:
...
## Painter's algorithm + profundidad del jugador (debt-close)

El orden de dibujo correcto en proyección 2:1 es **`col + row + z` ascendente**
(de atrás a delante). `GameRenderer` recoge todas las tiles, las ordena por esa
clave y, antes de dibujar cada tile, comprueba si el jugador debe intercalarse.

La clave del jugador se calcula mediante **muestreo de 5 puntos** del rombo
marcador (centro + extremos N/E/S/W en píxeles de pantalla): cada punto se
reproyecta al espacio mundo usando `Math.round` para evitar saltos espúreos en
celdas enteras, y se toma la celda más al sur (`max(col + row)`). Esto
garantiza que cualquier tile visualmente delante del jugador siempre se dibuje
encima de él.

Corrección asociada en `IsoProjection` javadoc: la fórmula documentaba
`- z * TILE_HEIGHT` cuando el paso real es `- z * Z_STEP_PX` (16 px, no 32 px).
...
| `render/GameRenderer.java` | painter's algorithm + player depth sorting |
| `projection/IsoProjection.java` | usa `Z_STEP_PX` + `projectCorner`; javadoc corregido |
