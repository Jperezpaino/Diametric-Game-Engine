...
public final class GameEngine {
    ...
    /**
     * Round-trips the default Skin / Material / Tile registries through the
     * Phase 8e JSON I/O kernel ({@link SkinSetIO}, {@link MaterialSetIO},
     * {@link TileSetIO}). Verifies the serialised output parses back into the
     * same number of entries; failures are logged but never crash the engine.
     */
    private static void smokeCheckResourceIo() {
        try {
            ...
            if (skinsIn == skinsOut && materialsIn == materialsOut && tilesIn == tilesOut) {
                System.out.println("[GameEngine] Phase 8e resource I/O round-trip OK ("
                    + skinsOut + " skins, " + materialsOut + " materials, "
                    + tilesOut + " tile defs, formatVersion=" + SkinSetIO.FORMAT_VERSION + ").");
            } else {
                System.err.println("[GameEngine] Phase 8e round-trip count mismatch: "
                    + "skins " + skinsIn + "->" + skinsOut + ", "
                    + "materials " + materialsIn + "->" + materialsOut + ", "
                    + "tiles " + tilesIn + "->" + tilesOut + ".");
            }
        } catch (final RuntimeException ex) {
            System.err.println("[GameEngine] Phase 8e round-trip failed: " + ex.getMessage());
            ex.printStackTrace(System.err);
        }
    }
}
