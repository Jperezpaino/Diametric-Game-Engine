...
import es.noa.rad.map.io.MapResourceIO;
...
public final class GameEngine {
    ...
    private static void logMapResourceSnapshot(final TileMap loaded) {
        try {
            ...
            roundTripResourceAsV2(resource);
        } catch (final RuntimeException ex) {
    ...
    /**
     * Phase 9d round-trip: serialise the loaded {@link MapResource} as a v2
     * JSON document, parse it back through {@link MapResourceIO}, then load
     * the same JSON through {@link MapLoader} (exercising the v2 dispatch
     * path that bridges back to a runtime {@link TileMap}). Confirms that
     * the new on-disk format faithfully represents whatever the legacy v1
     * loader produced from {@code map_*.json}.
     */
    private static void roundTripResourceAsV2(final MapResource resource) {
        try {
            final MapResource withStructure = new MapResource(
                resource.width(), resource.depth(), resource.length(),
                resource.tiles(),
                java.util.List.of(new StructureInstance("smoke_marker", 0, 0, 0)));
            final String v2Json = MapResourceIO.toJson(withStructure);
            final MapResource reparsed = MapResourceIO.parse(v2Json);
            final TileMap viaV2  = MapLoader.parse(v2Json);

            final boolean ok =
                   reparsed.tiles().size()      == withStructure.tiles().size()
                && reparsed.structures().size() == withStructure.structures().size()
                && viaV2.getWidth() == resource.width()
                && viaV2.getDepth() == resource.depth();
            if (ok) {
                System.out.println("[GameEngine] Phase 9d MapResource v2 round-trip OK ("
                    + reparsed.tiles().size()      + " tiles, "
                    + reparsed.structures().size() + " structures, "
                    + "formatVersion=" + MapResourceIO.FORMAT_VERSION + ").");
            } else {
                System.err.println("[GameEngine] Phase 9d MapResource v2 round-trip count mismatch: "
                    + "tiles "      + withStructure.tiles().size()      + "->" + reparsed.tiles().size() + ", "
                    + "structures " + withStructure.structures().size() + "->" + reparsed.structures().size() + ", "
                    + "reload size " + viaV2.getWidth() + "x" + viaV2.getDepth() + ".");
            }
        } catch (final RuntimeException ex) {
            System.err.println("[GameEngine] Phase 9d MapResource v2 round-trip failed: " + ex.getMessage());
            ex.printStackTrace(System.err);
        }
    }
}
