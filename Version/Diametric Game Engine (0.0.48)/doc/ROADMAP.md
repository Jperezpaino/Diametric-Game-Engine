...
- **9b** `StructureSet` JSON format (`formatVersion: 1`). ✅ Done — new [StructureSetIO](src/main/java/es/noa/rad/map/io/StructureSetIO.java) sits on top of the Phase 8e `Json` kernel: a top-level `{ formatVersion, structures: [...] }` document, where each structure carries `id`, `name` and a nested `tiles: [{tileId, x, y, z}]` array (relative coords). Rejects unknown `formatVersion`. Empty structure lists serialise as `"structures": []`. The boot-time smoke check now also round-trips a synthetic 3-tile pillar and logs `[GameEngine] Phase 8e/9b resource I/O round-trip OK (… 1 structures with 3 tiles …)`.
- **9c** Map model v2 — supports both `TileInstance` and `StructureInstance`; absolute coords; formatVersion: 1. ✅ Done — added the pure-domain record [MapResource](src/main/java/es/noa/rad/map/MapResource.java) (`width`, `depth`, `length`, `List<TileInstance> tiles`, `List<StructureInstance> structures`) with positive-dimension validation and defensive `List.copyOf`. The static factory `fromTileMap(TileMap)` walks the runtime grid and exports each voxel as a `TileInstance` carrying its `definitionId()` (Phase 8d-bis made that always available). The boot-time validation now logs `[GameEngine] Phase 9c MapResource snapshot: 15x15x3, 273 tile instances, 0 structure instances` so any regression in the conversion is immediately visible. I/O lives in 9d.
- **9d** New `MapLoader` / `MapWriter` — handle the v2 format with a backward-compat shim that auto-migrates the legacy `map_N.json` palette format. ✅ Done — added [MapResourceIO](src/main/java/es/noa/rad/map/io/MapResourceIO.java) (v2 wire format: `formatVersion: 2`, `width/depth/length`, dense `tilePalette` of `tileId` strings, dense `tileLayer` of palette indices with `-1` for empty cells, sparse `structureInstances`). [MapLoader](src/main/java/es/noa/rad/map/io/MapLoader.java) now auto-detects `formatVersion` (defaulting to legacy v1 when absent) and dispatches: v1 → existing palette+indices parser (legacy `map_*.json` keep loading byte-identically), v2 → `MapResourceIO.parse` then bridged to a runtime `TileMap` through `TileDefinitionRegistry.DEFAULT`. Structure instances on v2 maps are not expanded yet (no `StructureRegistry` is wired into the loader); a one-time stderr notice signals their presence. The boot-time check now logs `[GameEngine] Phase 9d MapResource v2 round-trip OK (273 tiles, 1 structures, formatVersion=2)`, proving the loaded v1 map serialises cleanly to v2, parses back to an equivalent `MapResource`, and reloads through the v2 dispatch path of `MapLoader`. Legacy `MapWriter` stays v1-only — saving a runtime map as v2 goes through `MapResourceIO.toJson(MapResource.fromTileMap(map))`.
...
- **10a** Parent POM + child module `diametric-engine` (current code, minus any editor-only class). ✅ Done — converted the project to a Maven multi-module reactor: the root [pom.xml](pom.xml) is now `packaging=pom` (`groupId=es.noa.rad`, `artifactId=diametric-parent`), declares `<modules>diametric-engine</modules>` and centralises plugin versions through `pluginManagement` (clean, resources, compiler, jar, exec). All previous source code lives unchanged under [diametric-engine/src](diametric-engine/src) with its own [diametric-engine/pom.xml](diametric-engine/pom.xml) (`packaging=jar`, inherits parent, declares the `Application` main class for `mvn -pl diametric-engine exec:java` and the manifest of the assembly jar). Build entrypoint stays a single `mvn -q clean compile` from the project root; running the game is now `mvn -q -pl diametric-engine exec:java`. Validated: reactor compiles clean, runtime boots normally with all Phase 8e/9b/9c/9d smoke logs intact and 60 FPS.
- **10b** Shared `diametric-tools-common` module — Swing utilities, resource browser widgets, JSON kernel glue. ✅ Done — added the [diametric-tools-common](diametric-tools-common/pom.xml) module to the reactor, depending on `diametric-engine` (one-way edge: editors → engine, never the reverse). Seeded with [SwingFrames](diametric-tools-common/src/main/java/es/noa/rad/tools/common/swing/SwingFrames.java) — façade for the `JFrame` boilerplate every editor will repeat: `applyDefaults(frame, title, minSize)` (sets title, `EXIT_ON_CLOSE`, the spec-mandated 640×480 `DEFAULT_MIN_SIZE`, and centers the window) plus `centerOnScreen(window)` (uses `GraphicsEnvironment.getMaximumWindowBounds` so taskbars are honoured). Headless-safe so it survives any future CI smoke. Resource browser widgets and JSON glue will land here as the editor sub-phases need them. Validated: reactor compiles cleanly with both modules and the engine boots unchanged (60 FPS, all Phase 8–9 smoke logs intact).
- **10c** Per-tool runnable modules: `tile-builder`, `structure-builder`, `map-builder`. ✅ Done — three sibling Maven modules added to the reactor:
  - [tile-builder](tile-builder/pom.xml) &rarr; `es.noa.rad.tools.tilebuilder.TileBuilderApp` (UI-001 host).
  - [structure-builder](structure-builder/pom.xml) &rarr; `es.noa.rad.tools.structurebuilder.StructureBuilderApp` (UI-002 host).
  - [map-builder](map-builder/pom.xml) &rarr; `es.noa.rad.tools.mapbuilder.MapBuilderApp` (UI-003 host).
  Each module depends only on `diametric-tools-common` (which transitively pulls in `diametric-engine`), declares its own `mainClass` for `maven-jar-plugin` and `exec-maven-plugin`, and ships a placeholder `App.java` that opens an empty centered 640×480 frame using `SwingFrames.applyDefaults`. UI implementation lands in Phases 11–13.
  Dev workflow: from the project root, run `mvn -q clean install -DskipTests` once after pulling fresh changes (populates the local m2 with the inter-module artifacts), then `mvn -q -pl <module> exec:java` launches any single runnable. Verified: reactor builds 6 modules SUCCESS, and all four runnables (engine + 3 editors) start without errors.
- **10d** Game runtime keeps depending only on `diametric-engine`; editor classes are unreachable from the game classpath. ✅ Done — verified by inspection: [diametric-engine/pom.xml](diametric-engine/pom.xml) declares zero `<dependency>` blocks on `diametric-tools-common`, `tile-builder`, `structure-builder` or `map-builder`. The dependency arrow is one-way: editors &rarr; tools-common &rarr; engine. The engine boots end-to-end (60 FPS, all Phase 8–9 smoke checks intact) without any editor module on the classpath.
