...
public final class SoundManager {
    ...
    /** Toggles global mute. When unmuted, the previously requested music track is restarted. */
    public void setMuted(final boolean newMuted) {
        if (this.muted == newMuted) return;
        this.muted = newMuted;
        if (newMuted) {
            stopMusic();
        } else if (musicId != null) {
            final Clip clip = clips.get(musicId);
            if (clip != null) {
                clip.setFramePosition(0);
                clip.loop(Clip.LOOP_CONTINUOUSLY);
                musicClip = clip;
